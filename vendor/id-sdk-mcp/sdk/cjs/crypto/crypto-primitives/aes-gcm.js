...
class AesGcm {
    ...
    static async decrypt(options) {
        ...
        const plaintextBuffer = await crypto_1.crypto.subtle.decrypt(algorithm, webCryptoKey, data);
    ...
    static async encrypt(options) {
        ...
        const algorithm = (additionalData === undefined)
            ? { name: 'AES-GCM', iv, tagLength }
            : { name: 'AES-GCM', additionalData, iv, tagLength };
        const ciphertextBuffer = await crypto_1.crypto.subtle.encrypt(algorithm, webCryptoKey, data);
        // Convert from ArrayBuffer to Uint8Array.
    ...
    static async importKey(key) {
        return crypto_1.crypto.subtle.importKey('raw', key.buffer, { name: 'AES-GCM', length: key.byteLength * 8 }, true, ['encrypt', 'decrypt']);
    }
}
