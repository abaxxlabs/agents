...
class Secp256k1 {
     ...
     */
    static async convertPublicKey(options) {
    ...
    static async generateKeyPair(options) {
        ...
        const publicKey = secp256k1_1.secp256k1.getPublicKey(privateKey, compressedPublicKey);
        const keyPair = {
     ...
     */
    static async getCurvePoints(options) {
        let { key } = options;
        ...
        const x = (0, utils_1.numberToBytesBE)(point.x, 32);
        const y = (0, utils_1.numberToBytesBE)(point.y, 32);
        return { x, y };
    }
    /**
     ...
     */
    static async getPublicKey(options) {
     ...
     * simplifies matters.
     */
    static async sharedSecret(options) {
        let { privateKey, publicKey } = options;
        // Compute the shared secret between the public and private keys.
        const sharedSecret = secp256k1_1.secp256k1.getSharedSecret(privateKey, publicKey);
    ...
    static async sign(options) {
        ...
        const signatureObject = secp256k1_1.secp256k1.sign(digest, key);
     ...
     */
    static async validatePrivateKey(options) {
        const { key } = options;
        return secp256k1_1.secp256k1.utils.isValidPrivateKey(key);
    }
    /**
     ...
     */
    static async validatePublicKey(options) {
        const { key } = options;
        try {
            // Decode Weierstrass points from key bytes.
            const point = secp256k1_1.secp256k1.ProjectivePoint.fromHex(key);
    ...
    static async verify(options) {
        ...
        const isValid = secp256k1_1.secp256k1.verify(signature, digest, key, { lowS: false });
        return isValid;
    }
}
