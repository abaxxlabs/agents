...
class CryptoAlgorithm {
    checkAlgorithmName(options) {
        ...
        if (algorithmName !== this.name) {
            throw new errors_js_1.NotSupportedError(`Algorithm not supported: '${algorithmName}'`);
        }
    }
    ...
    checkKeyAlgorithm(options) {
        ...
        if (keyAlgorithmName && keyAlgorithmName !== this.name) {
            throw new errors_js_1.InvalidAccessError(`Algorithm '${this.name}' does not match the provided '${keyAlgorithmName}' key.`);
        }
    }
    checkKeyType(options) {
        ...
        if (keyType && keyType !== allowedKeyType) {
            throw new errors_js_1.InvalidAccessError(`Requested operation is not valid for the provided '${keyType}' key.`);
        }
    }
    checkKeyUsages(options) {
        ...
        if (!keyUsages.every(usage => allowedUsages.includes(usage))) {
            throw new errors_js_1.InvalidAccessError(`Requested operation(s) '${keyUsages.join(', ')}' is not valid for the provided key.`);
        }
    }
    /**
