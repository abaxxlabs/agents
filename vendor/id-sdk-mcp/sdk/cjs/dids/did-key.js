...
class DidKeyMethod {
    static async create(options) {
        ...
        if ((_b = (_a = keySet.verificationMethodKeys) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.publicKeyJwk) {
            ...
            multibaseId = await index_js_2.Jose.jwkToMultibaseId({ key: publicKeyJwk });
        }
        if (!multibaseId) {
    ...
    static async createDocument(options) {
        ...
        let multibaseValue;
        try {
            ({ id: multibaseValue } = (0, utils_js_1.parseDid)({ didUrl: did }));
        }
        catch (error) {
        ...
        const verificationMethodTypes = (0, utils_js_1.getVerificationMethodTypes)({ didDocument: document });
     ...
     */
    static async createEncryptionMethod(options) {
         ...
         */
        const { key: rawPublicKeyBytes, multicodecCode: multicodecValue, } = await DidKeyMethod.deriveEncryptionKey({ multibaseValue });
        /**
         ...
         */
        const kemMultibaseValue = index_js_2.utils.keyToMultibaseId({
        ...
        if (publicKeyFormat === 'JsonWebKey2020') {
            const jwkParams = await index_js_2.Jose.multicodecToJose({ code: multicodecValue });
            const jsonWebKey = await index_js_2.Jose.keyToJwk(Object.assign({ keyMaterial: rawPublicKeyBytes, keyType: 'public' }, jwkParams));
     ...
     */
    static async deriveEncryptionKey(options) {
         ...
         * header.
         */
        const { key: rawPublicKeyBytes, multicodecCode: multicodecValue } = index_js_2.utils.multibaseIdToKey({ multibaseKeyId: multibaseValue });
        /**
        ...
        if (multicodecValue === 0xed) {
            const generatedPublicEncryptionKeyBytes = await index_js_2.Ed25519.convertPublicKeyToX25519({
     ...
     */
    static async createSignatureMethod(options) {
        const { did, enableExperimentalPublicKeyTypes, multibaseValue, publicKeyFormat } = options;
        /**
         * 1. Initialize verificationMethod to an empty object.
         */
         ...
         */
        const { key: rawPublicKeyBytes, multicodecCode: multicodecValue, multicodecName } = index_js_2.utils.multibaseIdToKey({ multibaseKeyId: multibaseValue });
        /**
         * 3. Ensure the proper key length of rawPublicKeyBytes based on the
        ...
        switch (multicodecName) {
            case 'secp256k1-pub':
                isValid = await index_js_2.Secp256k1.validatePublicKey({ key: rawPublicKeyBytes });
                break;
            case 'ed25519-pub':
                isValid = await index_js_2.Ed25519.validatePublicKey({ key: rawPublicKeyBytes });
                break;
            case 'x25519-pub':
    ...
    static async generateKeySet(options) {
        ...
        switch (keyAlgorithm) {
            case 'Ed25519': {
                keyPair = await new index_js_2.EdDsaAlgorithm().generateKey({
            ...
            case 'secp256k1': {
                keyPair = await new index_js_2.EcdsaAlgorithm().generateKey({
        ...
        const publicKeyJwk = await index_js_2.Jose.cryptoKeyToJwk({ key: keyPair.publicKey });
        const privateKeyJwk = await index_js_2.Jose.cryptoKeyToJwk({ key: keyPair.privateKey });
        const keySet = {
    ...
    static validateIdentifier(options) {
        const { did } = options;
        const { method, id: multibaseValue } = (0, utils_js_1.parseDid)({ didUrl: did });
        ...
        return (scheme !== 'did' ||
            method !== 'key' ||
            parseInt(version) > 0 ||
            (0, index_js_1.universalTypeOf)(multibaseValue) !== 'String' ||
            !multibaseValue.startsWith('z'));
    }
}
exports.DidKeyMethod = DidKeyMethod;
/**
