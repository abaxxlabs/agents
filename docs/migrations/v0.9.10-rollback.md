...
If you are unsure which case you were on pre-v0.9.10.0, check git for the bootstrap call site immediately before the v0.9.10.0 migration commit. The migration doc's environment audit (`docs/migrations/byok.md` § "Environment audit") is the same procedure inverted.
...
1. Reapply the v0.9.10.0 bootstrap changes (case #1, #2, or #4 from `docs/migrations/byok.md` per your current state).
2. `npm install --save-exact @abaxxlabs/agents@0.9.10.0` (or the patch version that resolved the issue).
3. Restart.

There is no DB rollback to undo, so re-applying is a normal forward upgrade. Re-run the post-migration checklist from `docs/migrations/byok.md` § "After the migration."
...
- Migration guide forward: `docs/migrations/byok.md`
- CHANGELOG entry for v0.9.10.0: `CHANGELOG.md` § `[0.9.10.0]`
- Database migrations 007 and 008 are not rolled back by this application rollback procedure.
