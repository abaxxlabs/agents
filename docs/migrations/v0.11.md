...
import { VcVerifier, AuditLogger, asMasterKey, createJwt, verifyJwtSignature, decodeJwt, encrypt, decrypt } from '@abaxxlabs/agents';
...
There is no `@abaxxlabs/agents/crypto` subpath. Crypto primitives are exported from the root package even though their internal implementations are split across `src/crypto/`, `src/encryption/`, `src/storage/`, and `src/auth/` by responsibility.
