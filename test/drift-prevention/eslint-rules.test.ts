  ...
  rules: {
    'no-restricted-syntax': [
      ...
      // Rule 1 — ban 'AGENTS_MASTER_KEY' literal
      {
        selector: "Literal[value='AGENTS_MASTER_KEY']",
        message:
          "[BYOK D16] Do not reference 'AGENTS_MASTER_KEY' as a string literal in.",
      },
      // Rule 1b — ban template literal
      {
        selector:
          "TemplateLiteral[expressions.length=0][quasis.0.value.cooked='AGENTS_MASTER_KEY']",
        message:
          "[BYOK D16] Do not reference 'AGENTS_MASTER_KEY' as a template literal in.",
      },
      ...
      // Other env reads remain allowed in server scope.
      {
        ...
        message:
          '[BYOK D16] Do not read process.env.AGENTS_MASTER_KEY directly in.',
      },
      ...
      // Selector matches Property nodes with key=Identifier name='AGENTS_MASTER_KEY'.
      {
        ...
        message:
          '[BYOK D16] Do not destructure AGENTS_MASTER_KEY from process.env (or define it as a property name) in.',
      },
    ],
  },
  languageOptions: {
