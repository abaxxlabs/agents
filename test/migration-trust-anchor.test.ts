...
/**
 * MigrationTrustAnchor — Unit Tests
 *
 * Why these tests matter: the MigrationTrustAnchor is the build-time-rooted
 * trust list MigrationExecutor consults before accepting a migration credential.
 * It closes the open-core defensibility gap surfaced by the architecture
 * audit -- without it, a
 * forker setting `AGENTS_TRUSTED_SERVERS=did:dht:fork-org-issuer` could
 * self-issue a migration credential and rebind agents to their own DID.
 *
 * The tests below verify the public contract:
 *   - Default construction reads from build-time OFFICIAL_MIGRATION_ISSUERS
 *     (empty in OSS source — safe-by-default).
 *   - The only runtime mutation path is `addFromParentCredentialChain()`.
 *   - There is no env-driven, constructor-driven, or admin-API-driven
 *     mutation surface (validated structurally — the class has no such
 *     methods or constructor arguments).
 *   - Source labels are correct (`'baked'` vs `'parent-credential-chain'`).
 *   - Re-adding a `'baked'` DID via parent-chain is a no-op (does not
 *     downgrade source).
 *   - Invalid input throws explicitly rather than silently accepting.
 */
